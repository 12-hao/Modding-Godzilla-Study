...
function doSystemTest()
{
    ...
    $resStr=$resStr.output("mysql_connect() available", function_exists("mysql_connect"), array("Yes", "No"));
    $resStr=$resStr.output("mysqli_connect() available", function_exists("mysqli_connect"), array("Yes", "No"));
...
$_POST["q[]"]=array("SET NAMES 'utf8mb4'",$_POST["q[]"]);
...
if (!$testMenu){
    ...
    if (!function_exists("mysql_connect") && !function_exists("mysqli_connect")) {
        EchoHeader(203);
        navicatEcho(GetBlock("MySQL not supported on the server"));
        return navicatGetEchoAll();
    }
    ...
    if ($use_mysqli) {
        ...
        if($errno_c > 0) {
            EchoHeader($errno_c);
            navicatEcho(GetBlock(mysqli_connect_error($conn)));
            return navicatGetEchoAll();
        }
        ...
        EchoHeader($errno_c);
        if($errno_c > 0) {
            navicatEcho(GetBlock(mysqli_error($conn)));
        } elseif($_POST["actn"] == "C") {
            EchoConnInfo($conn);
        } elseif($_POST["actn"] == "Q") {
            for($i=0;$i<count($_POST["q[]"]);$i++) {
                ...
                if($errno > 0)
                    ...
                    if($numfields > 0) {
                        ...
                        if(phpversion_int() >= 40300)
                            navicatEcho(GetBlock(mysqli_info($conn)));
                        else
        ...
        EchoHeader($errno_c);
        if($errno_c > 0) {
            navicatEcho(GetBlock(mysql_error()));
        } elseif($_POST["actn"] == "C") {
            EchoConnInfo($conn);
        } elseif($_POST["actn"] == "Q") {
            for($i=0;$i<count($_POST["q[]"]);$i++) {
                ...
                if($errno > 0)
                    ...
                    if($numfields > 0) {
                        ...
                        if(phpversion_int() >= 40300)
                            navicatEcho(GetBlock(mysql_info($conn)));
                        else
                            navicatEcho(GetBlock(""));
                    }
                }
